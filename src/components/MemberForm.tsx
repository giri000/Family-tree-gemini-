      ...
      <div className="bg-white sm:rounded-2xl shadow-xl sm:border border-slate-100 max-w-2xl w-full h-[100dvh] sm:h-auto sm:max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 border-b pb-1">Personal Details</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">Color Badge Aspect</label>
                  ...
                  {AVATAR_COLORS.map((c) => (
                    <button
                      ...
                      className={`w-6 h-6 rounded-full border cursor-pointer transition-transform ${c.className} ${
                        avatarColor === c.className ? 'ring-2 ring-indigo-500 ring-offset-2 scale-110' : 'border-slate-300 opacity-80'
                      }`}
                    />
                  ))}
                </div>
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 border-b pb-1">Chronologies & Milestones</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="flex items-center gap-1 text-xs font-semibold text-slate-600 mb-1">
                  <Calendar className="w-3.5 h-3.5 text-slate-400" /> Birth Date
                </label>
                <input
                  id="input-birth-date"
                  type="date"
                  ...
                  className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20"
                />
              </div>
              <div>
                <label className="flex items-center gap-1 text-xs font-semibold text-slate-600 mb-1">
                  <MapPin className="w-3.5 h-3.5 text-slate-400" /> Birthplace
                </label>
                <input
            ...
            <div className="flex items-center gap-2 bg-slate-50 p-3 rounded-lg border border-slate-100">
              ...
              />
              <label htmlFor="checkbox-deceased" className="text-xs font-semibold text-slate-700 cursor-pointer select-none">
            ...
            {isDeceased && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Death Date</label>
                  <input
                    id="input-death-date"
                    type="date"
                    value={deathDate}
                    onChange={(e) => setDeathDate(e.target.value)}
                    className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 animate-fade-in"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Death Location</label>
                  <input
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 border-b pb-1">Family & Kinship Connections</h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Father</label>
                <select
                  id="select-father"
                  value={fatherId}
                  onChange={(e) => setFatherId(e.target.value)}
                  className="w-full px-2.5 py-2 text-sm rounded-lg border border-slate-200 bg-white focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20"
                >
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Mother</label>
                <select
                  id="select-mother"
                  value={motherId}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Spouse / Partner</label>
                <select
                  id="select-spouse"
                  value={spouseId}
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 border-b pb-1">Contact & AI Agent Context</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
            ...
            <div className="grid grid-cols-1 gap-4 bg-indigo-50/50 p-4 rounded-xl border border-indigo-100/50 mt-2">
              <div>
                <label className="block text-xs font-semibold text-indigo-900 mb-1">Aliases / Known As</label>
                <input
                  type="text"
                  value={aliases}
                  onChange={(e) => setAliases(e.target.value)}
                  placeholder="e.g. Mom, Nana, Ted, The Boss (used by AI to identify them)"
                  className="w-full px-3 py-2 text-sm rounded-lg border border-indigo-200/60 bg-white focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-indigo-900 mb-1">AI Context & Instructions</label>
                <textarea
                  value={aiContext}
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 border-b pb-1">Biography & Heritage notes</h4>
            <div className="grid grid-cols-1 gap-4">
              <div>
                <label className="flex items-center gap-1 text-xs font-semibold text-slate-600 mb-1">
                  <Briefcase className="w-3.5 h-3.5 text-slate-400" /> Primary Vocation / Profession
                </label>
                <input
                  id="input-occupation"
                  type="text"
              ...
              <div>
                <label className="flex items-center gap-1 text-xs font-semibold text-slate-600 mb-1">
                  <FileText className="w-3.5 h-3.5 text-slate-400" /> Life Memoirs & Anecdotes
                </label>
                <textarea
                  id="textarea-notes"
                  value={notes}
