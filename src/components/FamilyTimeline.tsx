...
export function FamilyTimeline({ members, onFocusMember }: FamilyTimelineProps) {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      {/* Filters bar */}
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between p-4 bg-white border border-slate-100 rounded-xl shadow-xs">
        {/* Search */}
      ...
      {sortedEvents.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-xl border border-dashed border-slate-200">
          ...
          <p className="text-sm font-medium text-slate-500">No chronological timeline achievements found.</p>
          <p className="text-xs text-slate-400 mt-1">Try modifying your text search query or adding birth dates to your family member profiles.</p>
        </div>
      ) : (
            ...
            return (
              <div key={`${event.member.id}-${event.type}-${event.year}-${idx}`} className="relative pl-6 md:pl-8 group">
                ...
                <div className="absolute -left-6 md:-left-32 top-0.5 md:w-24 text-right hidden md:block">
                  <span className="text-lg font-mono font-bold text-slate-800 tracking-tight">
                    {event.year}
                  </span>
                ...
                <div className={`absolute -left-[9px] top-1.5 w-4 h-4 rounded-full border-2 flex items-center justify-center transition-transform group-hover:scale-125 ${
                  ...
                  <div className={`w-2 h-2 rounded-full ${isBirth ? 'bg-indigo-600' : 'bg-slate-600'}`} />
                </div>

                {/* Event Card */}
                <div className="bg-white border hover:border-slate-300 rounded-xl p-4 shadow-xs hover:shadow-sm transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-sm font-bold text-slate-850 hover:text-indigo-650 cursor-pointer flex items-center gap-1" onClick={() => onFocusMember(event.member.id)}>
                        {event.member.firstName} {event.member.lastName}
                        <ArrowUpRight className="w-3 h-3 text-slate-400 shrink-0" />
                      </h4>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                        isBirth ? 'bg-indigo-50 text-indigo-700 border border-indigo-100' : 'bg-slate-100 text-slate-700'
                      }`}>
                    ...
                    {event.member.notes && (
                      <p className="text-xs text-slate-405 italic text-slate-400 border-l border-slate-100 pl-2 mt-1">
                        "{event.member.notes}"
                      </p>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className="text-[11px] font-semibold text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-1.5 self-start md:self-auto cursor-pointer"
                  >
                    View in Tree
                  </button>
                </div>
