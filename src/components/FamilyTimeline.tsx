...
export function FamilyTimeline({ members, onFocusMember }: FamilyTimelineProps) {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      {/* Filters bar */}
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl shadow-xs">
        {/* Search */}
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20"
          />
        </div>
          ...
          <div className="flex bg-slate-100 dark:bg-slate-800 p-0.5 rounded-lg border border-slate-200 dark:border-slate-700">
            <button
              ...
              className={`px-3 py-1 rounded-md transition-all ${
                sortOrder === 'desc'
                  ? 'bg-white dark:bg-slate-700 shadow-xs text-indigo-700 dark:text-indigo-400'
          ...
          {sortedEvents.map((event, idx) => {
            ...
            return (
              <div key={`${event.member.id}-${event.type}-${event.year}-${idx}`} className="relative pl-6 md:pl-8 group">
                ...
                {/* Event Card */}
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-600 rounded-xl p-4 shadow-xs hover:shadow-sm transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-1.5">
                    {/* Tiny year node for small/mobile devices */}
                    <div className="flex items-center gap-1.5 md:hidden">
                      <span className="text-sm font-mono font-bold text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">
                        {event.year}
                      </span>
                      {event.dateStr && (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                        isBirth ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
                      }`}>
                    ...
                    {event.member.notes && (
                      <p className="text-xs text-slate-400 border-l border-slate-100 dark:border-slate-700 pl-2 mt-1">
                        "{event.member.notes}"
                      </p>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className="text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/40 border border-indigo-100 dark:border-indigo-800 rounded-lg px-3 py-1.5 self-start md:self-auto cursor-pointer"
                  >
                    View in Tree
                  </button>
                </div>
